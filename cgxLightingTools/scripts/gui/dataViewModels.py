...
class DataTableModel(QtCore.QAbstractTableModel):
    ...
    def flags(self, index):
        '''
        Set flags for each table cell.
        Re-implement if different flags are needed and use conditional chains
        if per-cell flags are needed.
    ...
    def headerData(self, section, orientation, role):
        ...
        if orientation != QtCore.Qt.Horizontal:
            return None
        if role not in [QtCore.Qt.DisplayRole, QtCore.Qt.ToolTipRole, QtCore.Qt.EditRole]:
            return None
    ...
    def insertRows(self, position, count, data, parent=QtCore.QModelIndex()):
        ...
        if manyRows:
            ...
            self.endInsertRows()
            return True
        elif len(data) == self.columnCount():
            self.beginInsertRows(parent, position, position + (count - 1))
...
class ObjectsListModel(QtCore.QAbstractListModel):
    ...
    def flags(self, index):
        '''
        Set flags of each item.  Re-implement if different
        flags are needed and use conditionalchains if per-item flags are needed.
        :param index: Item to be filtered
        :type index: QModelIndex
        :return: Flags to be used
        :rtype: QtCore.Qt.Flags
    ...
    def insertRows(self, position, rows, parent=QtCore.QModelIndex()):
        ...
        :rtype: boolean
        '''
        if len(rows) <= 0 or position > len(self.dataList) or position < 0:
            return False
...
class TreeNode(object):
    ...
    def child(self, row):
        ...
        '''
        return None if row > len(self.children) - 1 else self.children[row]
    ...
    def removeColumns(self, indexList):
        ...
        if self.parent is not None and self.parent.columnCount() != self.columnCount() - len(indexList):
            return False
        for i in sorted(indexList, reverse=True):
            del(self.data[i])
            del(self.headers[i])
    ...
    def log(self, tabLevel=-1):
        ...
        output = "".join("\t" for _ in range(tabLevel))
...
class DataTreeModel(QtCore.QAbstractItemModel):
    ...
    # --------------------------------------------------------------------------------------------
    def __init__(self,
                 root=TreeNode(['name', 'itemType', 'internalName'],
                               ['name', 'itemType', 'internalName']),
                 headers=None,
                 parent=None):
        '''
    ...
    def flags(self, index):
        '''
        Set flags for each item.
        Re-implement if different flags are needed and use conditional
         chains if per-item flags are needed.
        :param index: Item to be filtered
        :type index: QModelIndex
        :return: Flags to be used
        :rtype: QtCore.Qt.Flags
    ...
    def index(self, row, column, parent=QtCore.QModelIndex()):
        ...
        return self.createIndex(row, column, childNode) if childNode else QtCore.QModelIndex()
    ...
    def insertColumns(self, position, columns, parent=QtCore.QModelIndex()):
        ...
        :rtype: boolean
        '''
        if position < 0 or type(columns) is not list:
            return False
        self.beginInsertColumns(parent, position, position + len(columns) - 1)
        for i in columns[::-1]:
            self.headers.insert(position, i)
